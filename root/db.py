...
class Database:
        ...
        try:
            ...
            self.cursor.execute('''
                CREATE TABLE IF NOT EXISTS clients (
                    ...
                    public_key TEXT UNIQUE, -- Публичный ключ OpenVPN/WireGuard (используется для OpenVPN/WireGuard)
    ...
    def add_user(self, user_id, username):
        ...
        try:
            # При добавлении нового пользователя, по умолчанию делаем его "подписанным"
    ...
    def add_client(self, user_id, client_name, public_key):
        """Добавляет новую VPN-конфигурацию для пользователя."""
        try:
            self.cursor.execute(
                "INSERT INTO clients (user_id, client_name, public_key, is_active) VALUES (?, ?, ?, ?)",
                (user_id, client_name, public_key, True) # По умолчанию активен
            )
            self.conn.commit()
            logging.info(f"Client {client_name} added for user {user_id}.")
            return True
        except sqlite3.Error as e:
    ...
    def get_user_clients(self, user_id):
        ...
        try:
            self.cursor.execute("SELECT id, client_name, public_key, is_active FROM clients WHERE user_id = ?", (user_id,))
            clients = self.cursor.fetchall()
            return [{'id': c[0], 'client_name': c[1], 'public_key': c[2], 'is_active': bool(c[3])} for c in clients]
        except sqlite3.Error as e:
    ...
    def get_client_by_id(self, client_id):
        ...
        try:
            self.cursor.execute("SELECT id, user_id, client_name, public_key, is_active FROM clients WHERE id = ?", (client_id,))
    ...
    def get_client_by_name(self, client_name):
        """Получает данные клиента по его имени (редко используется напрямую)."""
        try:
            self.cursor.execute("SELECT id, user_id, client_name, public_key, is_active FROM clients WHERE client_name = ?", (client_name,))
            client_data = self.cursor.fetchone()
            if client_data:
                return {
                    'id': client_data[0],
            ...
            logging.error(f"Error getting client by name {client_name}: {e}")
            return None
